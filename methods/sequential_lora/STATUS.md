# Sequential LoRA

Status: official O-LoRA baseline source referenced; project-local baseline implementation/configs added in `source/project_local/`.

Official reference source: https://github.com/cmnfriend/O-LoRA

Official reference commit: `07117e1fc4a5f5ad9308a815a42cee8f46502dc8`

Project-local source commit: `d711d912926c58c13acc02b3c3e4cfddd9f2c969`

No separate official Sequential LoRA repository was verified.
